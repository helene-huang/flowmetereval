...
from data.cicids2017.engelen_latest.load import FEATURE_COLUMNS_DUPS, FEATURE_COLUMNS_NO_DUPS
...
def load_cicids2017_hhuang_fix(feature_list: list[str] | None=None) -> tuple[pd.DataFrame, pd.Series, pd.Series]:

    csv_path: Path = CURRENT_DIR / "cicids2017.csv"
    ...
    if "Packet Length Variance" in feature_list:  # config engelen_corrected_values_with_duplicates
        df_.loc[:,"Packet Length Variance"] = df_["Packet Length Std"]**2

    if "Average Packet Size" in feature_list:
        df_.loc[:, "Average Packet Size"] = df_["Packet Length Mean"]
